...
public class SearchController : Controller
{
    ...
    public IActionResult Index(string? q, string? category, DateTime? start, DateTime? end, string? availability, string? sort)
    {
        ...
        if (start.HasValue)
        {
            query = query.Where(e => e.DateTime >= start.Value);
        }

        if (end.HasValue)
        {
            query = query.Where(e => e.DateTime <= end.Value.AddDays(1));
        }
    ...
    // AJAX live search used on the home page
    [HttpGet]
    public IActionResult Live(string? q)
    {
        ...
        return PartialView("~/Views/Shared/_EventPartial.cshtml", results);
    }
}
